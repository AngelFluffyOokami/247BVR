...
var installText = "Choose your install method:" +
	"\n\nWindows Service: " +
	"\nCreates a new user and installs as a Windows service, controllable via service management, installs an automatic updater service alongside." +
	"\nSupports: Start|Stop|Pause|Continue" +
	"\nStart: Starts program execution. (Slow)" +
	"\nStop: Stops program execution. (Slow)" +
	"\nPause: Pauses execution by disconnecting from websocket and disabling all discord handlers, except a resume handler (Fast)" +
	"\nContinue: Continues execution by reconnecting to websocket and enabling all discord handlers. (Fast)" +
	"\n\nInstall as current user" +
	"\nInstalls as current user to any directory specified." +
	"\nMust be run manually, or added to startup programs file."
